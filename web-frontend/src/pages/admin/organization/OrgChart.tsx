      ...
      >
        ...
        <div className="px-6 pb-6">
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex gap-3">
            <Button
              fullWidth
              onClick={() => alert("Chức năng nhắn tin đang phát triển")}
            >
              Nhắn tin
            </Button>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-slate-900">
            {dept ? "Chỉnh sửa Phòng ban" : "Thêm Phòng ban con"}
          </h3>
...
export const OrgChart = () => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Xóa phòng ban này và tất cả phòng ban con?")) {
      ...
      const findChildren = (parentId: string) => {
        ...
        departments
          .filter((d) => d.parentDeptId === parentId)
  ...
  const handleSaveDept = (data: Partial<Department>) => {
    if (editingDept) {
      ...
      setDepartments((prev) =>
        prev.map((d) =>
          d.id === editingDept.id ? ({ ...d, ...data } as Department) : d
        )
      );
      setEditingDept(null);
      ...
      const newDept: Department = {
        ...
        managerAvatar:
          "https://ui-avatars.com/api/?name=New+Manager&background=random",
        memberCount: 0,
        ...
        >
          ...
          <div
            className={`absolute top-0 left-0 w-1 h-full rounded-l-lg ${
              children.length > 0 ? "bg-brand-500" : "bg-slate-300"
            }`}
          ></div>
          ...
          <div className="flex items-center gap-3">
            {!isCompact && (
              <img
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  handleProfileClick(dept.managerName);
                }}
              />
            )}
        ...
        {children.length > 0 && (
          <>
            ...
            <div className="flex gap-8 relative">
              ...
              {children.length > 1 && (
                <div
                  className="absolute top-0 left-[calc(50%-50%+144px)] right-[calc(50%-50%+144px)] h-px bg-slate-300"
                  style={{
                    left: isCompact
                      ? "calc(50%-50%+80px)"
                      : "calc(50%-50%+144px)",
  ...
  return (
    <div className="animate-fadeIn h-full flex flex-col relative">
      ...
      <div className="flex justify-between items-center mb-4 z-10 relative pointer-events-none">
        <div className="pointer-events-auto">
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Nhấn giữ chuột để di chuyển. Cuộn chuột để phóng to/thu nhỏ.
          </p>
        </div>
      ...
      >
        ...
        >
          <div className="flex justify-center">{renderNode("bod")}</div>
        </div>
      </div>
    </div>
  );

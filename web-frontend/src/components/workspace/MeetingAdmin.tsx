...
import { 
    ...
    MapPin, Video, RefreshCw, Monitor, ArrowLeft, 
    Copy, Check, Lock, Globe, Shield, Calendar, Clock
} from 'lucide-react';
...
const INITIAL_ROOMS: MeetingRoom[] = [
    { 
        ...
        accessLevel: 'Private',
        allowedDepartments: ['Ban Giám Đốc (Board of Directors)', 'Khối Công Nghệ (Technology)'],
...
const RoomForm = ({ room, onSave, onCancel }: RoomFormProps) => {
    ...
    const handleDeptToggle = (dept: string) => {
        ...
        if (currentDepts.includes(dept)) {
            setFormData({ ...formData, allowedDepartments: currentDepts.filter(d => d !== dept) });
        } else {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-fadeIn max-w-2xl mx-auto">
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <h3 className="font-bold text-slate-900">{room ? 'Chỉnh sửa Cuộc họp' : 'Tạo Cuộc họp mới'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <Input 
                    ...
                    placeholder={formData.type === 'Physical' ? "Ví dụ: Họp Giao ban - Phòng A" : "Ví dụ: Weekly Sync - Online"}
                    required 
                />
                ...
                {/* Room Type Selection */}
                <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Hình thức tổ chức</label>
                ...
                {formData.type === 'Physical' ? (
                    ...
                    <div className="animate-fadeIn p-4 bg-slate-50 rounded-lg border border-slate-100">
                         <label className="block text-sm font-semibold text-slate-700 mb-2">Đường dẫn họp (Tự động tạo)</label>
                         <div className="flex gap-2">
                            ...
                            <Button type="button" variant="outline" onClick={generateLink} title="Tạo mã mới">
                                <RefreshCw size={18} />
                            </Button>
                         </div>
                    </div>
                ...
                <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Quyền truy cập (Access Level)</label>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        ...
                        >
                            <div className={`p-2 rounded-full ${formData.accessLevel === 'Public' ? 'bg-green-200' : 'bg-slate-100'}`}>
                                <Globe size={18} />
                            </div>
                            <div className="text-left">
                                <span className="block font-medium text-sm">Công khai (Public)</span>
                                <span className="block text-xs opacity-75">Tất cả nhân viên</span>
                            </div>
                        </div>
                        ...
                        >
                            <div className={`p-2 rounded-full ${formData.accessLevel === 'Private' ? 'bg-slate-300' : 'bg-slate-100'}`}>
                                <Lock size={18} />
                            </div>
                            <div className="text-left">
                                <span className="block font-medium text-sm">Riêng tư (Private)</span>
                                <span className="block text-xs opacity-75">Chỉ định phòng ban</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {formData.accessLevel === 'Private' && (
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 animate-fadeIn">
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-3">Chọn phòng ban được phép tham gia:</label>
                                    ...
                                    <label key={dept} className="flex items-center gap-3 cursor-pointer hover:bg-white p-2 rounded transition-colors">
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                                            ...
                                            : 'bg-white border-slate-300'
                                             ...
                                             {formData.allowedDepartments?.includes(dept) && <Check size={14} className="text-white"/>}
                                        </div>
                                        <input 
                                            type="checkbox" 
                                            className="hidden"
                                            checked={formData.allowedDepartments?.includes(dept) || false}
                                            onChange={() => handleDeptToggle(dept)}
                                        />
                                        <span className={`text-sm ${formData.allowedDepartments?.includes(dept) ? 'text-brand-700 font-medium' : 'text-slate-600'}`}>
                                            {dept}
                                        </span>
                                    </label>
                                ))}
                ...
                <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Trạng thái hiện tại</label>
                    <select 
                ...
                <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
                    ...
                    <Button type="submit"><Save size={18} className="mr-2"/> Lưu & Thông báo</Button>
                </div>
            </form>
        </div>
    );
            ...
            {view === 'list' ? (
                <>
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h2 className="text-2xl font-bold text-slate-900">Quản lý Lịch họp</h2>
                            <p className="text-slate-500 mt-1">Danh sách các cuộc họp sắp tới và trạng thái phòng.</p>
                        </div>
                        <Button onClick={handleCreate}><Plus size={18} className="mr-2"/> Tạo cuộc họp mới</Button>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {rooms.map((room) => (
                            <div key={room.id} className="bg-white rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-all group overflow-hidden flex flex-col">
                                <div className="p-5 flex-1">
                                    <div className="flex justify-between items-start mb-3">
                                        <div className={`p-2 rounded-lg ${room.type === 'Online' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>
                                            {room.type === 'Online' ? <Video size={18} /> : <MapPin size={18} />}
                                        </div>
                                        <div className="flex gap-2">
                                            <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full flex items-center gap-1 border ${
                                                room.accessLevel === 'Public' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-slate-100 text-slate-700 border-slate-200'
                                            }`}>
                                                {room.accessLevel === 'Public' ? <Globe size={10}/> : <Lock size={10}/>}
                                    ...
                                    <h3 className="text-lg font-bold text-slate-900 mb-2 truncate" title={room.name}>{room.name}</h3>
                                    ...
                                    <div className="flex items-center gap-3 mb-3 text-sm bg-slate-50 p-2 rounded-lg border border-slate-100">
                                        <div className="flex items-center gap-1.5 text-slate-700 font-medium">
                                            ...
                                            {room.scheduleDate ? new Date(room.scheduleDate).toLocaleDateString('vi-VN') : '---'}
                                        </div>
                                    ...
                                    <div className="min-h-[40px]">
                                        {room.type === 'Physical' ? (
                                            <div className="text-sm text-slate-500 space-y-1">
                                                <p className="flex items-center gap-2"><MapPin size={14}/> {room.location}</p>
                                                <p className="flex items-center gap-2"><Users size={14}/> Sức chứa: {room.capacity} người</p>
                                            </div>
                                        ) : (
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded border border-slate-100 group-hover:border-blue-200 transition-colors">
                                                    ...
                                                    <span className="text-xs font-mono text-slate-700 truncate flex-1">{room.meetingLink}</span>
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); handleCopy(room.meetingLink || '', room.id); }}
                                    ...
                                    {/* Private Access List */}
                                    {room.accessLevel === 'Private' && room.allowedDepartments && room.allowedDepartments.length > 0 && (
                                        <div className="mt-3 pt-3 border-t border-slate-100">
                                            <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Tham gia:</p>
                                            <div className="flex flex-wrap gap-1">
                                                {room.allowedDepartments.slice(0, 2).map((dept, i) => (
                                                    <span key={i} className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded truncate max-w-[120px]">
                                                        {dept.split('(')[0].trim()}
                                                    </span>
                                                ))}
                                ...
                                <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex gap-2 mt-auto">
                                    <Button variant="outline" className="flex-1 text-xs h-9 bg-white" onClick={() => handleEdit(room)}>
                                        <Edit2 size={14} className="mr-2"/> Chi tiết
                                    </Button>
                                    <Button variant="outline" className="text-xs h-9 px-3 text-red-600 hover:bg-red-50 border-slate-200 bg-white" onClick={() => handleDelete(room.id)}>
                                        <Trash2 size={16}/>
                                    </Button>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <span className="text-xs text-slate-400">Kiểm tra lịch trống & đặt phòng</span>
                        </div>
                    </div>
                </>
            ) : (

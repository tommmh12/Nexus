...
export const QuickChatWidget: React.FC = () => {
  ...
  useEffect(() => {
    if (activeUser) {
      setMessages([
        {
          id: "1",
          text: `Chào ${activeUser.name
            .split(" ")
            .pop()}, công việc thế nào rồi?`,
          sender: "me",
          time: "10:00",
        },
        {
          ...
          sender: "them",
          time: activeUser.time === "5p" ? "10:05" : "Yesterday",
        },
      ]);
    }
  }, [activeUser]);
  ...
  const filteredUsers = MOCK_CHAT_USERS.filter(
    (u) =>
          ...
          {activeUser ? (
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                />
                <div
                  className={`absolute bottom-0 right-0 w-2.5 h-2.5 border-2 border-indigo-600 rounded-full ${getStatusColor(
                    activeUser.status
                  )}`}
                ></div>
              </div>
              <div>
                ...
                <span className="text-[10px] opacity-80 block capitalize">
                  {activeUser.status}
                </span>
              </div>
            </div>

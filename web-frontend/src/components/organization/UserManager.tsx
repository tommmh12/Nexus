...
import { MOCK_USERS, MOCK_ACTIVITIES } from '../../data/mockData';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { 
    Plus, Edit2, Trash2, ArrowLeft, Mail, Phone, Clock, Shield, 
    Link as LinkIcon, CheckCircle2, Save, Key, User as UserIcon, 
    Activity, MessageSquare, FileText, CheckSquare, LogIn
} from 'lucide-react';

export const UserManager = () => {
    const [view, setView] = useState<'list' | 'detail' | 'form'>('list');
    const [users, setUsers] = useState<EmployeeProfile[]>(MOCK_USERS);
    ...
    const ActivityTimeline = ({ activities }: { activities: ActivityLog[] }) => {
        ...
        const getActivityIcon = (type: ActivityType) => {
            switch(type) {
                case 'post_create': return <FileText size={16} />;
                case 'comment': return <MessageSquare size={16} />;
                case 'task_complete': return <CheckSquare size={16} />;
        ...
        const getActivityColor = (type: ActivityType) => {
             switch(type) {
                case 'post_create': return 'bg-blue-100 text-blue-600 border-blue-200';
                case 'comment': return 'bg-orange-100 text-orange-600 border-orange-200';
                case 'task_complete': return 'bg-green-100 text-green-600 border-green-200';
            ...
            <div className="relative pl-4 border-l border-slate-200 space-y-6">
                {activities.map(act => (
                    <div key={act.id} className="relative group">
                         <div className={`absolute -left-[25px] top-1 w-8 h-8 rounded-full border-4 border-white flex items-center justify-center ${getActivityColor(act.type)}`}>
                         ...
                         <div className="bg-white p-4 rounded-lg border border-slate-100 shadow-sm hover:border-brand-200 hover:shadow-md transition-all">
                             <div className="flex justify-between items-start mb-1">
                                 <p className="text-sm font-semibold text-slate-900">{act.content}</p>
                                 <span className="text-xs text-slate-400 whitespace-nowrap">{act.timestamp}</span>
                             </div>
                             {act.target && (
                                 <p className="text-sm text-brand-600 font-medium mb-1">"{act.target}"</p>
                             )}
            ...
            <div className="animate-fadeIn">
                <div className="mb-6 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button onClick={() => handleEdit(user)}><Edit2 size={16} className="mr-2"/> Chỉnh sửa hồ sơ</Button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm text-center">
                            ...
                            <div className="mt-6 pt-6 border-t border-slate-100 space-y-3 text-left">
                                ...
                                <div className="flex items-center text-sm text-slate-600">
                                    <Phone size={16} className="mr-3 text-slate-400"/> {user.phone}
                                </div>
                                <div className="flex items-center text-sm text-slate-600">
                                    <Clock size={16} className="mr-3 text-slate-400"/> Gia nhập: {user.joinDate}
                                </div>
                            </div>
                        </div>
                         ...
                         <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm grid grid-cols-2 gap-4 text-center">
                             ...
                             <div>
                                 <p className="text-2xl font-bold text-slate-900">{user.karmaPoints || 0}</p>
                    ...
                    <div className="lg:col-span-8 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                        <div className="flex border-b border-slate-200 bg-slate-50">
                            <button onClick={() => setActiveTab('info')} className={`flex-1 py-4 text-sm font-medium border-b-2 ${activeTab === 'info' ? 'border-brand-600 text-brand-600 bg-white' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>Thông tin cá nhân</button>
                            <button onClick={() => setActiveTab('activity')} className={`flex-1 py-4 text-sm font-medium border-b-2 ${activeTab === 'activity' ? 'border-brand-600 text-brand-600 bg-white' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>Lịch sử hoạt động</button>
                            <button onClick={() => setActiveTab('security')} className={`flex-1 py-4 text-sm font-medium border-b-2 ${activeTab === 'security' ? 'border-brand-600 text-brand-600 bg-white' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>Tài khoản & Bảo mật</button>
                        </div>
                        ...
                        <div className="p-6 flex-1 bg-slate-50/30">
                            ...
                            {activeTab === 'info' && (
                                <div className="animate-fadeIn space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="bg-white p-4 rounded-lg border border-slate-200">
                                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Quản lý trực tiếp</p>
                                            <p className="font-medium text-slate-900">Trần Minh Đức</p>
                                        </div>
                                        <div className="bg-white p-4 rounded-lg border border-slate-200">
                                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Văn phòng làm việc</p>
                                            <p className="font-medium text-slate-900">Tầng 12, Tòa nhà Nexus</p>
                                        </div>
                                    </div>
                                    
                                    <div className="bg-white p-6 rounded-xl border border-slate-200">
                                        <h3 className="font-bold text-slate-900 mb-4 flex items-center">
                                            <LinkIcon size={18} className="mr-2 text-slate-500"/> Tài khoản liên kết
                                        </h3>
                                        <div className="space-y-4">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-slate-500">
                                                            {user.linkedAccounts.find(a => a.provider === 'google')?.connected ? 'Đã kết nối' : 'Chưa kết nối'}
                                                        </div>
                                                    </div>
                                                </div>
                                                {user.linkedAccounts.find(a => a.provider === 'google')?.connected && (
                                            ...
                                            <div className="flex items-center justify-between p-3 border border-slate-100 rounded-lg bg-slate-50">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="text-sm font-semibold text-slate-900">Microsoft 365</div>
                                                        <div className="text-xs text-slate-500">
                                                            {user.linkedAccounts.find(a => a.provider === 'microsoft')?.connected ? 'Đã kết nối' : 'Chưa kết nối'}
                                                        </div>
                                                    </div>
                                                </div>
                                                {user.linkedAccounts.find(a => a.provider === 'microsoft')?.connected && (
                                                    <CheckCircle2 size={16} className="text-green-500"/>
                                                )}
                                            </div>
                                        </div>
                            ...
                            {activeTab === 'activity' && (
                                <div className="animate-fadeIn">
                                    <div className="flex justify-between items-center mb-6">
                                        <h3 className="font-bold text-slate-900">Dòng thời gian hoạt động</h3>
                            ...
                            {activeTab === 'security' && (
                                <div className="animate-fadeIn space-y-6">
                                     <div className="bg-white p-6 rounded-xl border border-slate-200">
                                        <h3 className="font-bold text-slate-900 mb-4 flex items-center"><Shield size={18} className="mr-2 text-brand-600"/> Trạng thái bảo mật</h3>
                                        <div className="space-y-4 divide-y divide-slate-100">
                                            <div className="flex justify-between items-center pt-2">
                                                <div>
                                                    <p className="text-sm font-medium text-slate-900">Xác thực 2 yếu tố (2FA)</p>
                                                    <p className="text-xs text-slate-500">Tăng cường bảo mật cho tài khoản.</p>
                                                </div>
                                                <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded">Đã bật</span>
                                            </div>
                                            <div className="flex justify-between items-center pt-4">
                                                <div>
                                                    <p className="text-sm font-medium text-slate-900">Đổi mật khẩu</p>
                                                    <p className="text-xs text-slate-500">Lần cuối: 30 ngày trước</p>
                                                </div>
                                                <Button variant="outline" className="text-xs h-8">Yêu cầu đổi</Button>
                                            </div>
                                            <div className="flex justify-between items-center pt-4">
                                                <div>
                                                    <p className="text-sm font-medium text-slate-900">Phiên đăng nhập</p>
                                                    <p className="text-xs text-slate-500">Hiện có 2 phiên đang hoạt động.</p>
                                                </div>
                                                <Button variant="outline" className="text-xs h-8 text-red-600 hover:bg-red-50 hover:text-red-700 border-red-200">Đăng xuất tất cả</Button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div className="animate-fadeIn max-w-3xl mx-auto">
            <div className="mb-6 flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-500 text-sm">Điền đầy đủ thông tin hồ sơ và thiết lập tài khoản.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSaveUser} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-8 space-y-8">
                    {/* Basic Info */}
                    <div>
                        <h3 className="font-bold text-slate-900 mb-4 pb-2 border-b border-slate-100">Thông tin cá nhân & Liên hệ</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <Input label="Họ và tên" defaultValue={selectedUser?.fullName} required placeholder="Ví dụ: Nguyễn Văn A" />
                            <Input label="Email công ty" type="email" defaultValue={selectedUser?.email} required placeholder="name@company.com" />
                            <Input label="Số điện thoại" defaultValue={selectedUser?.phone} placeholder="09xx..." />
                            <Input label="Mã nhân viên" defaultValue={selectedUser?.employeeId} placeholder="NEX-000" />
                        </div>
                    </div>

                    {/* Job Info */}
                    <div>
                        <h3 className="font-bold text-slate-900 mb-4 pb-2 border-b border-slate-100">Thông tin công việc</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-2">Phòng ban</label>
                                <select className="w-full bg-slate-50 border border-slate-200 rounded-md p-2.5 text-sm focus:ring-2 focus:ring-brand-500 outline-none" defaultValue={selectedUser?.department}>
                            ...
                            <Input label="Chức danh / Vị trí" defaultValue={selectedUser?.position} placeholder="Ví dụ: Senior Developer" />
                            <Input label="Ngày gia nhập" type="date" defaultValue={selectedUser?.joinDate ? new Date(selectedUser.joinDate.split('/').reverse().join('-')).toISOString().split('T')[0] : ''} />
                        </div>
                    </div>

                    {/* Account Settings */}
                    <div>
                        <h3 className="font-bold text-slate-900 mb-4 pb-2 border-b border-slate-100">Thiết lập tài khoản</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-2">Vai trò hệ thống (Role)</label>
                                <select className="w-full bg-slate-50 border border-slate-200 rounded-md p-2.5 text-sm focus:ring-2 focus:ring-brand-500 outline-none" defaultValue={selectedUser?.role}>
                            ...
                            <div>
                                ...
                                <div className="flex gap-4 mt-2">
                                    <label className="flex items-center cursor-pointer">
                                        <input type="radio" name="status" defaultChecked={!selectedUser || selectedUser.status === 'Active'} className="mr-2 text-brand-600 focus:ring-brand-500" />
                                        <span className="text-sm text-slate-700">Active (Hoạt động)</span>
                                    </label>
                                    <label className="flex items-center cursor-pointer">
                                        <input type="radio" name="status" defaultChecked={selectedUser?.status === 'Blocked'} className="mr-2 text-brand-600 focus:ring-brand-500" />
                                        <span className="text-sm text-slate-700">Blocked (Khóa)</span>
                                    </label>
                                </div>
                            </div>
                            {!isEditing && (
                                <div className="md:col-span-2 bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-blue-600 mt-1">Mật khẩu ngẫu nhiên sẽ được gửi đến email của nhân sự sau khi tạo thành công.</p>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-slate-50 px-8 py-4 border-t border-slate-200 flex justify-end gap-3">
                    ...
                    <Button type="submit"><Save size={18} className="mr-2" /> {isEditing ? 'Lưu thay đổi' : 'Tạo nhân sự'}</Button>
                </div>
            </form>
        </div>
    );
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                    {MOCK_USERS.slice(0, 5).map((u) => (
                        <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    <img className="h-8 w-8 rounded-full object-cover border border-slate-200" src={u.avatarUrl} alt="" />
                                    <div className="ml-3">
                                        <div className="text-sm font-medium text-slate-900">{u.fullName}</div>

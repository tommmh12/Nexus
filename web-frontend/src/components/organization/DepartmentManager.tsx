...
import { MOCK_DEPARTMENTS, MOCK_USERS, MOCK_PROJECTS } from '../../data/mockData';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { 
    Plus, MoreHorizontal, Building, Users, Target, ArrowLeft, Edit2, 
    TrendingUp, Award, UserPlus, ArrowRightLeft, Trash2, X, Save,
    Search, Check
} from 'lucide-react';
...
const DepartmentFormModal = ({ department, onSave, onCancel, users }: DepartmentFormProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const managerAvatar = manager ? manager.avatarUrl : 'https://ui-avatars.com/api/?name=' + formData.managerName;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-slate-900">{department ? 'Chỉnh sửa Phòng ban' : 'Thêm Phòng ban mới'}</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Trưởng phòng</label>
                        <select 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Input label="Ngân sách (Budget)" value={formData.budget} onChange={e => setFormData({...formData, budget: e.target.value})} />
                        <div>
                             <label className="block text-sm font-semibold text-slate-700 mb-2">Trạng thái KPI</label>
                             <select 
                                className="w-full bg-slate-50 border border-slate-200 rounded-md p-2.5 text-sm outline-none focus:ring-2 focus:ring-brand-500"
                                value={formData.kpiStatus}
                                onChange={e => setFormData({...formData, kpiStatus: e.target.value as any})}
                             >
...
const TransferModal = ({ user, departments, onConfirm, onCancel }: TransferModalProps) => {
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                        <ArrowRightLeft size={20} className="text-brand-600"/> Điều chuyển Nhân sự
                    </h3>
                    <button onClick={onCancel}><X size={20} className="text-slate-400 hover:text-slate-600"/></button>
                </div>
                
                <p className="text-sm text-slate-600 mb-4">
                    Chọn phòng ban mới cho nhân viên <span className="font-bold text-slate-900">{user.fullName}</span>.
                </p>

                <div className="mb-6">
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Phòng ban đích</label>
                    <select 
                        className="w-full bg-slate-50 border border-slate-200 rounded-md p-2.5 text-sm outline-none focus:ring-2 focus:ring-brand-500"
                        value={targetDept}
                    ...
                    >
                        ...
                        {departments.filter(d => d.name !== user.department).map(d => (
...
const AddMemberModal = ({ department, availableUsers, onConfirm, onCancel }: AddMemberModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 flex flex-col max-h-[80vh]">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-bold text-slate-900">Thêm nhân sự vào {department.name}</h3>
                    <button onClick={onCancel}><X size={20} className="text-slate-400 hover:text-slate-600"/></button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto border border-slate-100 rounded-lg mb-4 p-2 space-y-1 custom-scrollbar">
                    {filteredUsers.length > 0 ? filteredUsers.map(u => (
                        <div 
                            ...
                            className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${selectedIds.includes(u.id) ? 'bg-brand-50 border border-brand-200' : 'hover:bg-slate-50 border border-transparent'}`}
                        >
                            <div className={`w-5 h-5 rounded border flex items-center justify-center ${selectedIds.includes(u.id) ? 'bg-brand-600 border-brand-600' : 'bg-white border-slate-300'}`}>
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">{u.department || 'Chưa phân bổ'}</p>
                            </div>
                        </div>
                    )) : (
                        <p className="text-center text-sm text-slate-500 py-4">Không tìm thấy nhân sự phù hợp.</p>
                    )}
                </div>

                <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                    <span className="text-sm text-slate-500">Đã chọn: <span className="font-bold text-slate-900">{selectedIds.length}</span></span>
                    <div className="flex gap-2">
                        <Button variant="ghost" onClick={onCancel}>Hủy</Button>
                        <Button disabled={selectedIds.length === 0} onClick={() => onConfirm(selectedIds)}>Thêm nhân sự</Button>
                    </div>
                </div>
            </div>
        </div>
...
const DepartmentDetailView = ({ 
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'members' | 'projects'>('members');
    const [transferUser, setTransferUser] = useState<EmployeeProfile | null>(null);
    ...
    const deptProjects = MOCK_PROJECTS.filter(p => 
        p.participatingDepartments.includes(department.name) || 
        (department.id === 'tech' && p.participatingDepartments.includes('Software Development'))
    );
        ...
        <div className="animate-fadeIn relative">
            ...
            {transferUser && (
                <TransferModal 
                    ...
                    onConfirm={(uid, target) => { onTransferUser(uid, target); setTransferUser(null); }} 
            ...
            {isAddModalOpen && (
                <AddMemberModal 
                    ...
                    onConfirm={(ids) => { onAddMembers(ids); setIsAddModalOpen(false); }} 
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start">
                        ...
                        <div className="flex-1">
                            <div className="flex justify-between items-start">
                                ...
                                <div className="flex gap-2">
                                    <Button variant="outline" onClick={onEdit}><Edit2 size={16} className="mr-2"/> Chỉnh sửa thông tin</Button>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-6 mt-6">
                                <div className="flex items-center gap-3 bg-slate-50 px-3 py-2 rounded-lg border border-slate-100">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-semibold text-slate-900 text-sm">{department.managerName}</p>
                                    </div>
                                    <button onClick={onEdit} className="ml-2 text-brand-600 text-xs hover:underline">Thay đổi</button>
                                </div>
                ...
                {activeTab === 'members' && (
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden animate-fadeIn">
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            ...
                            <Button className="text-xs h-9" onClick={() => setIsAddModalOpen(true)}>
                                ...
                                {deptMembers.length > 0 ? deptMembers.map(u => (
                                    <tr key={u.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <Button variant="ghost" className="text-xs h-8 text-blue-600 hover:bg-blue-50" onClick={() => setTransferUser(u)}>
                                                <ArrowRightLeft size={14} className="mr-1"/> Điều chuyển
                                            </Button>
                                        </td>
                                    </tr>
                                )) : (
                        ...
                        {deptProjects.length > 0 ? deptProjects.map(p => (
                             <div key={p.id} className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm hover:border-brand-300 transition-colors">
                                <div className="flex justify-between mb-2">
                                    <span className="text-xs font-bold text-brand-600 bg-brand-50 px-2 py-0.5 rounded">{p.code}</span>
                                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                        p.status === 'Done' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                    }`}>{p.status}</span>
                                </div>
...
export const DepartmentManager = () => {
    ...
    const handleTransferUser = (userId: string, targetDeptName: string) => {
        ...
        setDepartments(prev => prev.map(d => {
            if (d.name === targetDeptName) return { ...d, memberCount: d.memberCount + 1 };
            if (d.name === oldDeptName) return { ...d, memberCount: Math.max(0, d.memberCount - 1) };
            return d;
        }));
    };
    ...
    const handleAddMembers = (userIds: string[]) => {
        ...
        setDepartments(prev => prev.map(d => 
            d.id === selectedDept.id ? { ...d, memberCount: d.memberCount + userIds.length } : d
        ));
    };
    ...
    return (
        <div className="animate-fadeIn relative">
             ...
             <div className="flex justify-between items-center mb-6">
                ...
                <Button onClick={handleCreateClick}><Plus size={18} className="mr-2"/> Thêm phòng ban</Button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {departments.filter(d => !d.parentDeptId || d.parentDeptId === 'bod').map(dept => (
                    <div key={dept.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all group cursor-pointer" onClick={() => handleViewDetail(dept)}>
                        <div className="p-6">
                            ...
                            <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                                <div>
                                    ...
                                    <span className="font-semibold text-slate-800">{dept.memberCount}</span>
                                </div>
                                <div>
                                    <div className="flex items-center text-slate-500 mb-1">
                        ...
                        <div className="bg-slate-50 px-6 py-3 border-t border-slate-200 rounded-b-xl flex justify-between items-center text-xs">
                             ...
                             <button className="text-brand-600 font-medium hover:underline" onClick={(e) => { e.stopPropagation(); handleViewDetail(dept); }}>Chi tiết</button>
                        </div>
                    </div>
                ))}
                
                {/* Add New Card Placeholder */}
                <div onClick={handleCreateClick} className="border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center text-slate-400 hover:border-brand-400 hover:text-brand-600 hover:bg-brand-50/50 transition-all cursor-pointer min-h-[300px]">

...
const AddDepartmentModal = ({
  ...
  project: Project;
  onClose: () => void;
  onAdd: (deptName: string) => void;
}) => {
  const [selectedDept, setSelectedDept] = useState("");
  const availableDepts: any[] = []; // TODO: Fetch from API
...
export const ProjectDetailView = ({
  ...
  const participatingDepts: any[] = []; // TODO: Fetch from API
  ...
  const handleRemoveDepartment = (deptName: string) => {
    if (
      window.confirm(
        `Bạn có chắc chắn muốn xóa phòng ban "${deptName}" khỏi dự án?`
      )
    ) {
      setLocalProject((prev) => ({
        ...prev,
        participatingDepartments: prev.participatingDepartments.filter(
          (d) => d !== deptName
        ),
      }));
    }
  ...
  const handleUpdateReportStatus = (
    ...
    setReports((prev) =>
      prev.map((r) => (r.id === id ? { ...r, status, feedback } : r))
    );
        ...
        {activeTab === "departments" && (
          <div className="animate-fadeIn">
            <div className="flex justify-between items-center mb-4">
              ...
              <Button className="text-xs" onClick={() => setShowAddDept(true)}>

...
export const useEmployeeDashboard = (): UseEmployeeDashboardReturn => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedProjects: DashboardProject[] = (projectsData || [])
        .slice(0, 5)
        .map((p: any) => ({
          id: p.id,
          name: p.name || p.title,
    ...
    try {
      ...
      if (bookingsData.status === "fulfilled") {
        const todayBookings = (bookingsData.value || [])
          .filter((b: any) => {
            const bookingDate = new Date(b.startTime || b.booking_date)
              .toISOString()
              .split("T")[0];
      ...
      if (onlineMeetingsData.status === "fulfilled") {
        const todayOnline = (onlineMeetingsData.value || [])
          .filter((m: any) => {
            try {
              const dateValue = m.scheduledAt || m.startTime;
              if (!dateValue) return false;
              const meetingDate = new Date(dateValue);
              if (isNaN(meetingDate.getTime())) return false;
              return meetingDate.toISOString().split("T")[0] === today;
            } catch {
              return false;
            }
          })
    ...
    try {
      ...
      const mapped: DashboardNotification[] = (notifData || [])
        .slice(0, 5)
        .map((n: any) => ({
          id: n.id,
          title: n.title,
      ...
      setStats((prev) => ({
        ...prev,
        unreadNotifications: mapped.filter((n) => !n.isRead).length,
      }));
    } catch (err) {

...
import { 
    ArrowLeft, Calendar, Clock, ArrowRight, Share2, Printer, 
    Edit2, Trash2, Plus, Image, Save, X, Tag, Search, Filter,
    Eye, MoreVertical, CheckCircle, FileText, Layout, Maximize2,
    Grid, List as ListIcon, AlertCircle
} from 'lucide-react';
                    ...
                    <div className="max-w-3xl">
                        ...
                        <div className="flex items-center text-slate-300 text-sm gap-6">
                            <div className="flex items-center gap-2">
                                <img src={article.authorAvatar} alt="" className="w-6 h-6 rounded-full border border-white/30" />
                ...
                <div className="flex gap-10">
                    ...
                    <div className="w-64 hidden lg:block sticky top-8 h-fit space-y-6">
                        <div className="border-t border-slate-100 pt-6">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Chia sẻ bài viết</h4>
                            <div className="flex gap-2">
                                <Button variant="outline" className="flex-1"><Share2 size={16} className="mr-2"/> Share</Button>
                                <Button variant="outline" className="w-10 px-0"><Printer size={16}/></Button>
                            </div>
                        </div>
                    </div>
                </div>
...
export const NewsEditorModal = ({ article, onSave, onClose }: NewsEditorProps) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-white z-10">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900">{article ? 'Chỉnh sửa bài viết' : 'Soạn bài viết mới'}</h2>
                        <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
                            <span className="bg-slate-100 px-1.5 py-0.5 rounded font-mono text-slate-600">{article ? article.id : 'NEW'}</span>
                            <span>•</span>
                    ...
                    <div className="flex items-center gap-3">
                         ...
                         <Button type="submit" form="news-form" className="bg-brand-600 hover:bg-brand-700 shadow-md">
                ...
                <div className="flex-1 overflow-hidden">
                    <form id="news-form" onSubmit={handleSubmit} className="h-full flex flex-col lg:flex-row">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 border-r border-slate-200 custom-scrollbar bg-slate-50/30">
                             <div className="space-y-6 max-w-4xl mx-auto bg-white p-8 shadow-sm border border-slate-100 rounded-xl min-h-full">
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Tóm tắt (Sapo)</label>
                                    <textarea 
                                ...
                                <div className="space-y-2 flex-1 flex flex-col">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button type="button" className="text-xs text-brand-600 hover:underline flex items-center bg-brand-50 px-2 py-1 rounded"><Maximize2 size={12} className="mr-1"/> Toàn màn hình</button>
                                    </div>
                                    <textarea 
                        ...
                        <div className="w-full lg:w-[320px] bg-white overflow-y-auto p-6 space-y-6 custom-scrollbar border-l border-slate-200">
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2 pb-2 border-b border-slate-100">
                                    <Layout size={16} className="text-brand-600"/> Cài đặt xuất bản
                                </h3>
                                
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 block mb-1.5">Trạng thái</label>
                                    <select 
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 block mb-1.5">Chuyên mục</label>
                                    <select 
                                        className="w-full bg-slate-50 border border-slate-200 rounded-md p-2 text-sm outline-none focus:border-brand-500"
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 block mb-1.5">Ngày đăng</label>
                                    <input 
                                        type="text"
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 block mb-1.5">URL Ảnh</label>
                                    <input 
                                        type="text"
                                    ...
                                    {formData.coverImage ? (
                                        <div className="mt-2 rounded-lg overflow-hidden aspect-video border border-slate-200 relative group">
                                            ...
                                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-xs">Xem trước</div>
                                        </div>
                                    ) : (
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {formData.tags?.map(tag => (
                                        <span key={tag} className="inline-flex items-center bg-brand-50 border border-brand-100 text-brand-700 text-xs px-2 py-1 rounded-md font-medium">
                                            {tag}
                                            <button type="button" onClick={() => removeTag(tag)} className="ml-1 text-brand-400 hover:text-red-500"><X size={12}/></button>
                                        </span>
                                    ))}
                                </div>
                                ...
                                <p className="text-[10px] text-slate-400 mt-2">Nhấn Enter để thêm thẻ mới.</p>
                            </div>
        ...
        <div className="animate-fadeIn h-full flex flex-col relative">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">CMS - Hệ thống quản trị nội dung bản tin.</p>
                </div>
                <Button onClick={handleCreate}><Plus size={18} className="mr-2"/> Viết bài mới</Button>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
                     ...
                     <div className="flex bg-slate-100 p-1 rounded-lg">
                        {['All', 'Published', 'Draft'].map(status => (
                            <button 
                                ...
                                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${filterStatus === status ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                ...
                <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button 
                        ...
                        className={`p-2 rounded-md ${viewMode === 'list' ? 'bg-white shadow text-brand-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <ListIcon size={18} />
                    </button>
                    <button 
                        onClick={() => setViewMode('grid')}
                        className={`p-2 rounded-md ${viewMode === 'grid' ? 'bg-white shadow text-brand-600' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <Grid size={18} />
                    </button>
                </div>
            ...
            {viewMode === 'list' ? (
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex-1">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Bài viết</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Tác giả</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Trạng thái</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Chuyên mục</th>
                                ...
                                <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-6 py-4 w-[40%]">
                                        <div className="flex gap-4">
                                            ...
                                            <div>
                                                <div className="font-bold text-slate-900 line-clamp-2 mb-1 group-hover:text-brand-600 cursor-pointer" onClick={() => handleEdit(item)}>{item.title}</div>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm text-slate-700 font-medium">{item.authorName}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${
                                            item.status === 'Published' ? 'bg-green-50 text-green-700 border-green-100' : 
                                            item.status === 'Draft' ? 'bg-amber-50 text-amber-700 border-amber-100' : 'bg-slate-50 text-slate-700 border-slate-200'
                                        }`}>
                                            <div className={`w-1.5 h-1.5 rounded-full ${item.status === 'Published' ? 'bg-green-500' : item.status === 'Draft' ? 'bg-amber-500' : 'bg-slate-400'}`}></div>
                                            {item.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => handleEdit(item)} className="p-2 text-slate-500 hover:text-brand-600 hover:bg-brand-50 rounded-lg transition-colors border border-transparent hover:border-slate-200" title="Chỉnh sửa">
                                                <Edit2 size={16}/>
                                            </button>
                                            <button onClick={() => handleDelete(item.id)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-slate-200" title="Xóa">
                                                <Trash2 size={16}/>
                                            </button>
                                        </div>
                                    </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fadeIn">
                    {filteredNews.map(item => (
                        <div key={item.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden group hover:shadow-md transition-all flex flex-col">
                            <div className="relative aspect-video overflow-hidden">
                                ...
                                <div className="absolute top-2 right-2">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold shadow-sm ${
                                        item.status === 'Published' ? 'bg-green-500 text-white' : 'bg-amber-500 text-white'
                                    }`}>
                                        {item.status}
                                    </span>
                                </div>
                            ...
                            <div className="p-4 flex-1 flex flex-col">
                                ...
                                <h3 className="font-bold text-slate-900 mb-2 line-clamp-2 leading-snug cursor-pointer hover:text-brand-600" onClick={() => handleEdit(item)}>
                                    {item.title}
                                </h3>
                                <div className="mt-auto flex justify-between items-center pt-3 border-t border-slate-100">
                                    ...
                                    <div className="flex gap-1">
                                         <button onClick={() => handleEdit(item)} className="p-1.5 text-slate-400 hover:text-brand-600 hover:bg-slate-100 rounded">
                                            <Edit2 size={14}/>
                                        </button>
...
export const NewsModule = () => {
    ...
    const publishedNews = MOCK_NEWS.filter(n => n.status === 'Published');
            ...
            {view === 'list' ? (
                <div className="space-y-8 p-6">
                     <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-6">
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900 mb-2">Bản tin Công ty</h1>
                            <p className="text-slate-500">Cập nhật tin tức, sự kiện và thông báo mới nhất từ Nexus Corp.</p>
                        </div>
                        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-lg p-1.5 shadow-sm">
                            <Search size={18} className="text-slate-400 ml-2" />
                            <input type="text" placeholder="Tìm kiếm tin tức..." className="bg-transparent border-none outline-none text-sm py-1 w-64" />
                        </div>
                    </div>
                    ...
                    {publishedNews.length > 0 && (
                        ...
                        >
                            <img src={publishedNews[0].coverImage} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                            ...
                            <div className="absolute bottom-0 left-0 p-8 md:p-12 max-w-4xl">
                                ...
                                <p className="text-slate-200 text-lg mb-6 line-clamp-2 max-w-2xl">{publishedNews[0].summary}</p>
                                
                                <div className="flex items-center text-white/80 text-sm gap-6 font-medium">
                                    <div className="flex items-center gap-2">
                                        <img src={publishedNews[0].authorAvatar} alt="" className="w-6 h-6 rounded-full border border-white/30" />
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
                            <span className="w-1 h-6 bg-brand-600 rounded-full"></span> Tin mới nhất
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {publishedNews.slice(1).map(item => (
                                <div key={item.id} onClick={() => handleArticleClick(item)} className="group cursor-pointer flex flex-col h-full">

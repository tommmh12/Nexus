...
import { 
    Search, Filter, Trash2, Eye, FileText, MessageSquare, 
    CheckSquare, LogIn, Activity, UserCog, Database, Calendar, X,
    ChevronDown, ChevronRight, Server, Download
} from 'lucide-react';

const ActivityDetailModal = ({ log, onClose }: { log: ActivityLog, onClose: () => void }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fadeIn" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-slate-900">Chi tiết Hoạt động</h3>
                ...
                <div className="space-y-4">
                    <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 flex gap-4 items-start">
                        ...
                        <div>
                            <p className="font-semibold text-slate-900 text-lg">{log.content}</p>
                            {log.target && <p className="text-brand-600 font-medium">Đối tượng: {log.target}</p>}
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span className="block text-slate-500 text-xs uppercase font-bold mb-1">Người thực hiện</span>
                            <span className="font-medium text-slate-800">{log.userName || log.userId}</span>
                        </div>
                        <div>
                        ...
                        <div>
                            <span className="block text-slate-500 text-xs uppercase font-bold mb-1">Loại hành động</span>
                            <span className="bg-slate-100 px-2 py-0.5 rounded text-xs font-mono">{log.type}</span>
                        </div>
                        <div>
                            <span className="block text-slate-500 text-xs uppercase font-bold mb-1">IP Address</span>
                    ...
                    <div className="border-t border-slate-100 pt-4">
                        ...
                        >
                            {showRaw ? <ChevronDown size={14} className="mr-1"/> : <ChevronRight size={14} className="mr-1"/>}
...
export const AuditLogManager = () => {
    const [logs, setLogs] = useState<ActivityLog[]>(MOCK_ACTIVITIES);
    const [searchQuery, setSearchQuery] = useState('');
    const [filterType, setFilterType] = useState<string>('all');
    const [selectedLog, setSelectedLog] = useState<ActivityLog | null>(null);

    const handleDelete = (id: string) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa nhật ký này? Hành động này không thể hoàn tác.')) {
    ...
    const getActivityIcon = (type: ActivityType) => {
        switch(type) {
            case 'post_create': return <FileText size={16} className="text-blue-500" />;
            case 'comment': return <MessageSquare size={16} className="text-orange-500" />;
            case 'task_complete': return <CheckSquare size={16} className="text-green-500" />;
            case 'login': return <LogIn size={16} className="text-slate-500" />;
            case 'personnel_change': return <UserCog size={16} className="text-purple-500" />;
            case 'data_backup': return <Database size={16} className="text-teal-500" />;
            case 'system': return <Server size={16} className="text-gray-600" />;
        ...
        <div className="animate-fadeIn h-full">
            ...
            <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Theo dõi toàn bộ hành động của nhân viên trên hệ thống.</p>
                </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <div className="relative">
                        ...
                        <Filter size={16} className="absolute left-3 top-2.5 text-slate-400 pointer-events-none"/>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50">
                        <tr>
                            <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-12">Loại</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nội dung hành động</th>
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        {filteredLogs.map(log => (
                            <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <p className="text-sm font-medium text-slate-900">{log.content}</p>
                                    {log.target && <p className="text-xs text-brand-600 mt-0.5">{log.target}</p>}
                                </td>
                                <td className="px-6 py-4">
                                    <div className="text-sm text-slate-700 font-medium">{log.userName || log.userId}</div>
                                    <div className="text-xs text-slate-400 font-mono">{log.ipAddress}</div>
                                </td>

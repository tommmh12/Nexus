...
const MENU_ITEMS: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
        ...
        { id: 'forum-reader', label: 'Diễn đàn Nội bộ', badge: 5, badgeColor: 'bg-red-500' },
    ]
  },
  {
      id: 'communication',
      ...
      icon: MessageCircle,
      children: [
          { id: 'chat-manager', label: 'Quản trị Giao tiếp', badge: 3 },
      ]
  },
  {
      id: 'content-cms',
...
const ContentAdminDashboard = ({ initialTab = 'news' }: { initialTab?: 'news' | 'forum' }) => {
        ...
        <div className="h-full flex flex-col">
            <div className="flex border-b border-slate-200 bg-white px-6">
                <button 
                    ...
                    className={`py-4 px-6 text-sm font-medium border-b-2 transition-colors ${activeTab === 'news' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                    Quản lý Tin tức
                </button>
                <button 
                    onClick={() => setActiveTab('forum')}
                    className={`py-4 px-6 text-sm font-medium border-b-2 transition-colors ${activeTab === 'forum' ? 'border-brand-600 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                    Kiểm duyệt Diễn đàn
                </button>
            </div>
...
export const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const [expandedMenus, setExpandedMenus] = useState<string[]>(['dashboard', 'community', 'workspace', 'communication', 'moderation', 'system']);
  ...
  const renderContent = () => {
      switch(activeMenu) {
          ...
          case 'event-manager': return <EventManager />;
          
          // Reader Views
          case 'forum-reader': return <ForumModule />;
          case 'news-reader': return <NewsModule />;

          // Admin Views
          case 'cms-news': return <NewsManager />;
          case 'cms-forum': return <ForumManager />;
          
          // Chat Module
          case 'chat-manager': return <ChatManager />;

          // System & Safety
          case 'audit-logs': return <AuditLogManager />;
          case 'alert-manager': return <AlertManager />;
  ...
  const isFullWidthView = ['cms-news', 'cms-forum', 'chat-manager'].includes(activeMenu);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      >
        {/* Sidebar Header */}
        <div className={`h-16 flex items-center ${isSidebarCollapsed ? 'justify-center px-0' : 'px-6'} bg-slate-950 border-b border-slate-800 transition-all duration-300`}>
          <div className="flex items-center gap-2 font-bold text-xl text-white tracking-tight overflow-hidden whitespace-nowrap">
             <div className="h-8 w-8 min-w-[32px] bg-brand-600 rounded-lg flex items-center justify-center">N</div>
             {!isSidebarCollapsed && <span className="animate-fadeIn">NEXUS <span className="text-slate-500 font-normal text-sm ml-1">ADMIN</span></span>}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 custom-scrollbar">
            <div className="mb-6">
                {!isSidebarCollapsed && <p className="px-2 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 animate-fadeIn">Main Menu</p>}
                    ...
                    <div key={item.id}>
                        <button
                            ...
                            className={`
                                ...
                                ${activeMenu === item.id ? 'bg-brand-600 text-white shadow-lg shadow-brand-900/20' : 'hover:bg-slate-800 hover:text-white'}
                                ${isSidebarCollapsed ? 'justify-center' : ''}
                            `}
                            title={isSidebarCollapsed ? item.label : undefined}
                        >
                            <div className="flex items-center">
                                {item.icon && <item.icon size={20} className={`${!isSidebarCollapsed ? 'mr-3' : ''} ${activeMenu === item.id ? 'text-white' : 'text-slate-400'}`} />}
                            ...
                            {!isSidebarCollapsed && (
                                <div className="flex items-center">
                                    {item.badge && (
                                        <span className={`${item.badgeColor || 'bg-brand-500'} text-white text-[10px] px-1.5 py-0.5 rounded-full mr-2`}>
                                            {item.badge}
                                        </span>
                                    )}
                                    {item.children && (
                                ...
                                {item.children.map((child) => (
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${activeMenu === child.id ? 'text-brand-400 bg-slate-800/50' : 'text-slate-400 hover:text-white hover:bg-slate-800/30'}
                                        `}
                                    >
                                        <span className="truncate">{child.label}</span>
                                        {child.badge && (
                                            <span className={`${child.badgeColor || 'text-slate-400'} text-xs font-bold`}>
                                                {child.badge}
                                            </span>
                                        )}
                                    </button>
        ...
        <div className="pt-4 pb-4 border-t border-slate-800 bg-slate-900">
            ...
            <div className={`flex ${isSidebarCollapsed ? 'justify-center' : 'justify-end px-4'}`}>
                <button 
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="relative">
                <button 
                    ...
                    className={`p-2 rounded-full relative transition-colors ${showNotifications ? 'bg-blue-50 text-brand-600' : 'text-slate-400 hover:text-brand-600 hover:bg-blue-50'}`}
                >
                    <Bell size={20} />
                    {unreadCount > 0 && (
                        ...
                        <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-slate-100 z-20 overflow-hidden animate-fadeIn">
                            ...
                            <div className="max-h-[350px] overflow-y-auto">
                                {notifications.length > 0 ? notifications.map(notif => (
                                    <div key={notif.id} className={`p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors ${!notif.isRead ? 'bg-blue-50/30' : ''}`}>
                                        <div className="flex gap-3">
                                            {notif.actorAvatar ? (
                                                <img src={notif.actorAvatar} className="w-8 h-8 rounded-full border border-slate-100 flex-shrink-0" alt=""/>
                                            ) : (
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    {!notif.isRead && <span className="w-1.5 h-1.5 rounded-full bg-brand-500"></span>}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="p-2 border-t border-slate-100 text-center">
                                <button className="text-xs text-slate-500 hover:text-brand-600 font-medium w-full py-1">Xem tất cả thông báo</button>
                            </div>
                        </div>
                    </>
                )}
        ...
        {/* Content Area */}
        <main className={`flex-1 overflow-y-auto bg-slate-50 ${isFullWidthView ? 'p-0' : 'p-4 sm:p-6 lg:p-8'}`}>
            <div className={`mx-auto h-full ${isFullWidthView ? '' : 'max-w-7xl'}`}>
                {renderContent()}
            </div>
        </main>
      </div>
